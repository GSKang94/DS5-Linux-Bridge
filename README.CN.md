...
本固件利用手柄的物理静音键（Mute键）实现了免驱动的硬件麦克风静音功能。
- **Windows 下的表现：** 
  - 按下手柄的静音键会切换本地硬件级静音（在固件中将麦克风数据流置零，并开/关手柄橙色指示灯）。
  - 在 Windows 声卡控制面板中对麦克风进行静音/取消静音，会自动同步手柄物理橙色指示灯的状态。
  - **同步局限性：** 由于本适配器是免驱的，且没有通过 UAC 状态中断端点通知主机，因此按下手柄物理按键时无法反向控制 Windows 系统音量控制面板的静音复选框。如果 Windows 系统中已经将麦克风静音，此时按下手柄按键熄灭指示灯，尽管固件硬件层已取消静音，Windows 系统在软件层仍会丢弃音频输入。建议在使用物理按键前，确保 Windows 系统中的麦克风处于未静音状态。
- **Linux 下的表现：**
  - 完美原生适配内核 `hid-playstation` 驱动。驱动程序会拦截按键事件并控制指示灯。当检测到主机驱动处于活动状态时，本地的 UAC 同步会被自动旁路，防止状态冲突。
